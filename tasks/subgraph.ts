...
async function subgraphCodegen(_args: HardhatArguments, hre: HardhatRuntimeEnvironment) {
  const { CORE_CONTRACT_ADDRESS, TOKENS_CONTRACT_ADDRESS, START_BLOCK } = hre.contracts;
...
async function subgraphDeploy(args: { name: string }, hre: HardhatRuntimeEnvironment) {
  ...
  // make the node create blocks on an interval so subgraph can process arrivals in block handler
